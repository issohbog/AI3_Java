...
public class FileManager {
	...
	private static void fileList(String path) {
		...
		for (File subFile : fileList) {
			// 수정일자
			long update = subFile.lastModified();		// 최종 수정 일자  
	...
	private static void makeDirectory(String path) {
		...
		// mkdirs() 	: 생성할 경로의 상위 경로까지 디렉토리를 생성해준다. - 가능하면 이걸 쓰는게 좋음 
		if(file.mkdirs()) {
	...
	private static void createFile(String path, String fileName) {
		// TODO: FileWriter 객체를 사용해서 간단한 텍스트를 입력하여 지정한 파일로 출력하시오. 
		// 1. File 객체 생성 
	...
	public static void main(String[] args) {
		...
		do {
			...
			// 메뉴판 출력 
//			FileManager fm = new FileManager();  객체 생성 후 객체로 참조해서 인스턴드 메소드 사용하거나 
			menu(); 							// 메소드를 static으로 선언해서 객체 생성없이 클래스 메소드로 사용하기 
			...
			String path = sc.nextLine();			// 엔터 지우기 

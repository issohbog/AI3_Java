...
public class StreamArrayList {
	...
	public static void main(String[] args) {
		...
		// 에너지 드링크 
		productList.add( new Product("몬스터 에너지", 2300) );
		...
		productList.stream()
//				   .sorted(Comparator.comparing( p -> p.getName()))   아래와 동일한 기능! 
		...
		// 정렬기준 두개는 어떻게 할까? 고민해보기! 
		
		// count() - 스트림 요소 개수
		// * 이름이 4글자 이상인 상품의 개수 
		long count = productList.stream()
								.filter( p -> p.getName().length() >= 4 )
								.count(); 			// count위에 마우스 올리면 해당 메소드의 반환타입 나온다(long) 
		...
		int sum = productList.stream()
							 .filter( p -> p.getPrice() < 2000 )
							 .mapToInt(p -> p.getPrice())			// price만 가져와서 int로 반환까지 해줌 
							 .sum();
